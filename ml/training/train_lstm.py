#!/usr/bin/env python3
"""
NET-08: Entrenamiento del modelo LSTM
Predice congestion y fallas en horizonte t+5min.
...
def generate_synthetic_dataset(n_samples: int = 3000) -> tuple:
    """
    Clase 0: trafico normal  (utilizacion < 70%)
    Clase 1: congestion/falla (utilizacion > 85%)
    """
    np.random.seed(42)
    X, y = [], []
    for _ in range(n_samples):
        ...
        for t in range(WINDOW_SIZE):
            util      = np.clip(base_util + np.random.normal(0, 0.05), 0, 1)
            ...
            window.append([delta_rx, delta_tx, err_ratio, util])
        X.append(window)
        y.append(label)
...
def train():
    ...
    print('[LSTM] Generando dataset sintetico...')
    X, y = generate_synthetic_dataset(3000)
    print(f'[LSTM] Dataset: {X.shape} | positivos: {y.sum()} negativos: {len(y)-y.sum()}')
    ...
    print(f'[LSTM] Modelo guardado en {MODEL_PATH}')
    print(f'[LSTM] Metricas finales: {metrics}')
